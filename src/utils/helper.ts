...
export function paginate(array: any[], limit = 0, page = 1) {
...
export function getCurrentDateDD_MM_YYYY(format = '/'): string {
...
export function convertFullDateToInt(date: Date): {
  ...
  const time = +(date.getTime() / 1000).toFixed();
  const start = Math.round(time / SECONDS_IN_A_DAY) * SECONDS_IN_A_DAY;
  return {
    start,
...
export function getMillisecondBasePeriodTime(type): number {
  let milliseconds = 0;
  switch (type) {
    case 'day':
...
export function getThisWeekInterval(): { start: number; end: number } {
  const start = moment().startOf('isoWeeks');
...
export function omitPersonField(client) {
  return _.omit(client.toObject(), ['password', '_id']);
}
